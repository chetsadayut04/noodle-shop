...
export function FloatingCart({ lines, totalCount, totalPrice, onAdd, onRemove, onClear, onCheckout }: FloatingCartProps) {
  ...
  return <>
    {open && <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      <div className="relative z-10 max-h-[86vh] w-full max-w-md overflow-hidden rounded-t-3xl bg-card shadow-2xl sm:rounded-3xl border border-border">
        ...
        <div className="space-y-3 border-t border-border px-5 py-4"><div className="flex items-center justify-between"><span className="text-xs font-semibold text-muted-foreground">ยอดรวมทั้งหมด</span><span className="font-display text-2xl font-bold text-primary">{totalPrice} บาท</span></div><div className="flex gap-2"><button type="button" onClick={onClear} className="flex items-center justify-center gap-1.5 rounded-full bg-secondary border border-border px-4 py-3 text-xs font-bold text-secondary-foreground hover:bg-secondary/80 transition-colors"><Trash2 className="h-4 w-4" />ล้าง</button><button type="button" onClick={() => { setOpen(false); onCheckout() }} className="flex-1 rounded-full bg-primary py-3 font-display font-bold text-sm text-primary-foreground shadow-md shadow-primary/25 hover:bg-primary/90 transition-all active:scale-95">สั่งอาหาร</button></div></div>
      </div>
    </div>}
    <div className="fixed inset-x-0 bottom-0 z-40 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-2"><button type="button" onClick={() => setOpen(true)} className="mx-auto flex w-full max-w-md items-center justify-between gap-3 rounded-full bg-gradient-to-r from-primary via-primary to-amber-900 px-3.5 py-2.5 text-primary-foreground shadow-xl shadow-primary/30 border border-white/15 transition-transform active:scale-[0.98]"><span className="relative flex h-11 w-11 items-center justify-center rounded-full bg-white/20"><ShoppingBag className="h-5 w-5" /><span className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-amber-400 px-1 text-[11px] font-bold text-amber-950 shadow-xs">{totalCount}</span></span><span className="font-display text-base font-bold">ดูรายการในตะกร้า</span><span className="font-display text-lg font-bold">{totalPrice} บาท</span></button></div>
  </>
}
